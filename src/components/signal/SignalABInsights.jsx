...
export default function SignalABInsights({ siteId, projectId }) {
  ...
  const fetchInsights = async () => {
    try {
      setLoading(true)
      const res = await api.post('/.netlify/functions/signal-analyze-tests', {
        autoPromote: false
      })
  ...
  const handleAutoPromote = async (elementId) => {
    try {
      await api.post('/.netlify/functions/signal-analyze-tests', {
        elementId,
        autoPromote: true
      })
      await fetchInsights()
