...
const SignalAILogo = ({ size = 484, className = '', white = false }) => {
  const uniqueId = useId();
  const gradientId = `signal-gradient-${uniqueId}`;
  const center = 242.1;
  
  // Outer ring is at roughly radius 210 from center
  // Equal spacing: outer=210, middle=168, inner=126 (42px between each)
  ...
  return (
    ...
    >
      ...
      {/* Middle ring (thirds) - rotates counter-clockwise - BEHIND */}
      ...
      {/* Inner ring (halves) - rotates clockwise - BEHIND */}
      ...
      {/* Outer static ring with arrow only - inner rings removed */}
      <path
        fill={`url(#${gradientId})`}
