...
function ScoreCard({ label, value, icon: Icon, description }) {
  ...
  return (
    <div className={`rounded-2xl p-5 ${colors.bg} border border-[var(--glass-border)] backdrop-blur-sm`}>
      ...
      <div className="text-xs text-[var(--text-tertiary)]">{colors.label}</div>
      {description && (
...
function MetricCard({ name, value, target, score }) {
  ...
  return (
    <div className="bg-[var(--glass-bg)] backdrop-blur-sm rounded-xl p-4 border border-[var(--glass-border)]">
...
function IssueItem({ title, description, severity, recommendation }) {
  const severityConfig = {
    critical: { bg: 'border-l-[var(--accent-red)] bg-[var(--accent-red)]/5', icon: XCircle, iconColor: 'text-[var(--accent-red)]' },
    warning: { bg: 'border-l-[var(--accent-orange)] bg-[var(--accent-orange)]/5', icon: AlertTriangle, iconColor: 'text-[var(--accent-orange)]' },
    info: { bg: 'border-l-[var(--accent-blue)] bg-[var(--accent-blue)]/5', icon: Eye, iconColor: 'text-[var(--accent-blue)]' }
  }
  ...
  return (
    <div className={`border-l-4 ${config.bg} p-4 rounded-r-xl`}>
...
function SecurityCheckRow({ name, status }) {
  return (
    <div className="flex items-center justify-between p-3 bg-[var(--glass-bg)] rounded-xl border border-[var(--glass-border)]">
      <span className="text-sm text-[var(--text-primary)]">{name}</span>
      {status ? (
        <Badge className="bg-[var(--accent-green)]/10 text-[var(--accent-green)] border-0">
          <CheckCircle className="w-3 h-3 mr-1" />
          Enabled
        </Badge>
      ) : (
        <Badge className="bg-[var(--accent-red)]/10 text-[var(--accent-red)] border-0">
    ...
    <div className="min-h-screen bg-[var(--surface-page)]">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="font-semibold text-white/90">Uptrade Media</span>
              </div>
            ...
            {/* Grade Badge */}
            <div className={`w-28 h-28 md:w-32 md:h-32 rounded-2xl bg-gradient-to-br ${gradeColor} flex items-center justify-center shadow-2xl flex-shrink-0`}>
              <span className="text-5xl md:text-6xl font-bold text-white">{grade}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <Card className="bg-[var(--glass-bg)] backdrop-blur-xl border-[var(--glass-border)] overflow-hidden">
          ...
          <CardContent className="pt-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 mt-4">
              <div className="bg-[var(--glass-bg-inset)] rounded-xl p-4 text-center">
                <Timer className="w-6 h-6 mx-auto mb-2 text-[var(--text-secondary)]" />
                <div className="text-2xl font-bold text-[var(--text-primary)]">{formatMs(audit.ttiMs)}</div>
                <div className="text-xs text-[var(--text-tertiary)]">Time to Interactive</div>
              </div>
              <div className="bg-[var(--glass-bg-inset)] rounded-xl p-4 text-center">
                <Clock className="w-6 h-6 mx-auto mb-2 text-[var(--text-secondary)]" />
                <div className="text-2xl font-bold text-[var(--text-primary)]">{formatMs(audit.tbtMs)}</div>
                <div className="text-xs text-[var(--text-tertiary)]">Total Blocking Time</div>
              </div>
              <div className="bg-[var(--glass-bg-inset)] rounded-xl p-4 text-center">
                <TrendingUp className="w-6 h-6 mx-auto mb-2 text-[var(--text-secondary)]" />
                <div className="text-2xl font-bold text-[var(--text-primary)]">{formatMs(audit.speedIndexMs)}</div>
                <div className="text-xs text-[var(--text-tertiary)]">Speed Index</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {seoIssues.length > 0 && (
          <Card className="bg-[var(--glass-bg)] backdrop-blur-xl border-[var(--glass-border)] overflow-hidden">
            <CardHeader>
              <div className="flex items-center gap-2">
                <Search className="w-6 h-6 text-[var(--accent-blue)]" />
        ...
        <Card className="bg-gradient-to-br from-gray-900 to-gray-800 text-white overflow-hidden border-0">
          <CardHeader>
            <div className="flex items-center gap-2">
          ...
          <CardContent className="py-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a 
                ...
                className="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-secondary)] text-white font-bold rounded-xl hover:shadow-lg transition-all"
              >
              ...
              <a 
                ...
                className="inline-flex items-center justify-center px-8 py-4 bg-[var(--glass-bg-elevated)] text-[var(--text-primary)] font-bold rounded-xl border border-[var(--glass-border)] hover:bg-[var(--glass-bg-hover)] transition-all"
              >
        ...
        {/* Footer */}
        <footer className="text-center py-8 border-t border-[var(--glass-border)]">
          <div className="flex items-center justify-center gap-2 mb-4">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--brand-primary)] to-[var(--brand-secondary)] flex items-center justify-center">
              <span className="text-white font-bold text-sm">U</span>
            </div>
            <span className="font-semibold text-[var(--text-primary)]">Uptrade Media</span>
          </div>
      ...
      {showBackToTop && (
        <Button
          ...
          className="fixed bottom-6 right-6 rounded-full w-12 h-12 bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-secondary)] hover:shadow-lg shadow-md"
          size="sm"
        >
