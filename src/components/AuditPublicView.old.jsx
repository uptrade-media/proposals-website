  ...
  return (
    <div className="min-h-screen bg-[var(--surface-page)]">
      ...
      >
        ...
        {/* SEO Issues */}
        {seoIssues.length > 0 && (
          <motion.div id="seo-issues" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        ...
        {/* Security Analysis */}
        <motion.div id="security-issues" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        ...
        {performanceIssues.length > 0 && (
          <motion.div id="performance-issues" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        ...
        {accessibilityIssues.length > 0 && (
          <motion.div id="accessibility-issues" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                  ...
                  <>
                    {scores.performance < 70 && (
                      <motion.div 
                        ...
                        onClick={() => document.getElementById('performance-issues')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                      >
                    ...
                    {scores.seo < 80 && (
                      <motion.div 
                        variants={fadeInUp} 
                        className="flex items-start gap-4 bg-white/5 rounded-xl p-4 border border-white/5 cursor-pointer hover:bg-white/10 transition-colors"
                        onClick={() => document.getElementById('seo-issues')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                      >
                    ...
                    {scores.accessibility < 90 && (
                      <motion.div 
                        variants={fadeInUp} 
                        className="flex items-start gap-4 bg-white/5 rounded-xl p-4 border border-white/5 cursor-pointer hover:bg-white/10 transition-colors"
                        onClick={() => document.getElementById('accessibility-issues')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                      >
                    ...
                    {/* Show security action if any security headers are missing */}
                    {(securityIssues.csp === false || securityIssues.xFrameOptions === false || securityIssues.hsts === false) && (
                      <motion.div 
                        variants={fadeInUp} 
                        className="flex items-start gap-4 bg-white/5 rounded-xl p-4 border border-white/5 cursor-pointer hover:bg-white/10 transition-colors"
                        onClick={() => document.getElementById('security-issues')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                      >
                        ...
                        <div>
                          <h4 className="font-semibold text-white">Strengthen Security Headers</h4>
                          <p className="text-gray-400 text-sm mt-1 leading-relaxed">
                            Some security headers are missing. Adding CSP, X-Frame-Options, and HSTS headers will protect against common attacks.
                          </p>
                          <span className="text-xs text-[var(--brand-primary)] mt-2 inline-block">Click to see details ↓</span>
                        </div>
                      </motion.div>
        ...
        {businessImpact?.details?.length > 0 && (
              ...
              <CardContent>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {businessImpact.details.map((detail, i) => (
                    <div key={i} className={`p-4 rounded-xl border ${detail.severity === 'critical' ? 'bg-red-500/10 border-red-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}>
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-white font-medium">{detail.metric}</span>
                        <span className={`text-sm px-2 py-0.5 rounded ${detail.severity === 'critical' ? 'bg-red-500/20 text-red-300' : 'bg-amber-500/20 text-amber-300'}`}>
                          {detail.value}
                        </span>
                      </div>
                ...
                {(businessImpact.estimatedBounceIncrease > 0 || businessImpact.estimatedConversionLoss > 0) && (
                  <div className="mt-6 p-4 bg-white/5 rounded-xl border border-white/10">
                    <h4 className="text-white font-medium mb-2">Estimated Impact</h4>
        ...
        {/* Industry Comparison */}
        {industryComparison?.comparisons?.length > 0 && (
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                    ...
                    <div key={i} className="flex items-center gap-4">
                      ...
                      <div className="flex-1 relative">
                        <div className="h-3 bg-gray-700/50 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full ${comp.diff >= 0 ? 'bg-gradient-to-r from-green-500 to-emerald-400' : 'bg-gradient-to-r from-red-500 to-orange-400'}`}
                      ...
                      <span className={`w-16 text-right font-medium ${comp.diff >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  ...
                  {resources.heaviestImages?.length > 0 && (
                    <div>
                      <h4 className="text-white font-medium mb-3 flex items-center gap-2">
                        ...
                        Largest Images ({Math.round(resources.totals.images)} KB)
                      </h4>
                      <div className="space-y-2">
                        {resources.heaviestImages.slice(0, 3).map((img, i) => (
                          <div key={i} className="flex justify-between text-sm bg-white/5 rounded-lg p-2">
                            <span className="text-gray-400 truncate flex-1 mr-2" title={img.url}>{img.url.split('/').pop()?.substring(0, 30)}</span>
                            <span className="text-blue-400 font-medium">{img.sizeFormatted}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {resources.heaviestScripts?.length > 0 && (
                    <div>
                      <h4 className="text-white font-medium mb-3 flex items-center gap-2">
                        ...
                        Largest Scripts ({Math.round(resources.totals.scripts)} KB)
                      </h4>
                      <div className="space-y-2">
                        {resources.heaviestScripts.slice(0, 3).map((script, i) => (
                          <div key={i} className="flex justify-between text-sm bg-white/5 rounded-lg p-2">
                            <span className="text-gray-400 truncate flex-1 mr-2" title={script.url}>{script.url.split('/').pop()?.substring(0, 30)}</span>
                            <span className="text-yellow-400 font-medium">{script.sizeFormatted}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                        ...
                        {resources.thirdParty.slice(0, 4).map((res, i) => (
                          <div key={i} className="text-sm bg-red-500/10 border border-red-500/20 rounded-lg p-2">
                            <span className="text-gray-400 truncate block text-xs" title={res.url}>
                              {(() => { try { return new URL(res.url).hostname } catch { return res.url.substring(0, 20) } })()}
                            </span>
                    ...
                    <div key={i} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {opp.savings && <span className="text-cyan-400 text-sm font-medium whitespace-nowrap">{opp.savings}</span>}
                        </div>
        ...
        {aiInsights && (
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <Card className="bg-gradient-to-br from-violet-900/20 via-purple-900/10 to-fuchsia-900/20 backdrop-blur-xl border-violet-500/20 overflow-hidden">
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-violet-200/70">Expert insights generated by AI</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                {aiInsights.competitiveAnalysis && (
                  <div className="p-4 bg-white/5 rounded-xl border border-violet-500/20">
                    <h4 className="text-violet-300 font-medium mb-2">Competitive Analysis</h4>
                    <p className="text-gray-300 text-sm">{aiInsights.competitiveAnalysis}</p>
                  </div>
                )}
                {aiInsights.technicalDebt && (
                  <div className="p-4 bg-white/5 rounded-xl border border-violet-500/20">
                    <h4 className="text-violet-300 font-medium mb-2">Technical Debt</h4>
                    <p className="text-gray-300 text-sm">{aiInsights.technicalDebt}</p>
                  </div>
                )}
