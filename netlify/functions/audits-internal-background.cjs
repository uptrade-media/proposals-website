...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Run mobile PageSpeed and SEO checks in parallel
    const [mobile, seo] = await Promise.all([
      runPageSpeed(audit.target_url, 'mobile'),
      runSeoChecks(audit.target_url)
    ])
    
    // Check if PageSpeed call failed
    if (mobile?.error) {
      console.error('[audit-background] PageSpeed call failed:', mobile.error)
      ...
      await supabase
        ...
        .eq('id', auditId)
      return { statusCode: isRateLimit ? 429 : 500, body: JSON.stringify({ error: errorMessage, isRateLimit }) }
    }
    
    // Extract scores from mobile only
    const getScore = (obj, path) => 
      path.reduce((acc, key) => acc?.[key], obj)
    
    const performance = Math.round((getScore(mobile, ['lighthouseResult', 'categories', 'performance', 'score']) ?? 0) * 100)
    const lighthouseSeo = Math.round((getScore(mobile, ['lighthouseResult', 'categories', 'seo', 'score']) ?? 0) * 100)
    const accessibility = Math.round((getScore(mobile, ['lighthouseResult', 'categories', 'accessibility', 'score']) ?? 0) * 100)
    const bestPractices = Math.round((getScore(mobile, ['lighthouseResult', 'categories', 'best-practices', 'score']) ?? 0) * 100)
    
    // Extract Core Web Vitals for AI context (mobile only)
    ...
    for (const [key, audit] of Object.entries(mobileAudits)) {
    ...
    console.log('[audit-background] Updating database with complete status...')
    ...
    if (updateError) {
      console.error('[audit-background] Database update error:', updateError)
    } else {
      console.log('[audit-background] Audit complete and saved:', auditId, { performance, lighthouseSeo, accessibility, bestPractices, overallScore, grade })
    }
    
    // NOTE: No email sent for internal audits
    
    return { statusCode: 200, body: JSON.stringify({ success: true, auditId, grade }) }
